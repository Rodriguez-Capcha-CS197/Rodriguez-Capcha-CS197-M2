...
class SpectralKoopmanMLP(nn.Module):
    """
    Spectral Koopman MLP: replaces SwiGLU with lift-rotate-readout.

    1. Lift:    x -> SiLU(W_lift @ x)                    [d -> d_k]
    2. Rotate:  block-diagonal 2x2 eigenvalue rotation     [d_k -> d_k]
    3. Readout: W_readout @ z                              [d_k -> d]

    Saves ~1/3 of parameters vs SwiGLU by replacing the gate matrix
    with O(d_k) eigenvalue parameters.
    """
...
class SpectralKoopmanMLPGated(nn.Module):
    """Gated variant with a separate gate projection."""
    def __init__(self, d, expand=2.667, spectral_norm_gamma=True):
        super().__init__()
        self.d_k = ((int(d * expand) + 63) // 64) * 64
        self.spectral_norm_gamma = spectral_norm_gamma
